...
## Idle behavior leaf - simply waits and plays idle animation
...
func before_run(actor: Node, blackboard: Node) -> void:
	...
	print("[IdleLeaf] before_run called for actor: ", actor.name, " duration: ", duration)
	# play idle animation via passive API if present
	if "idle" in actor:
		actor.idle()
